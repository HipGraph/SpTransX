...
device = "cpu"   # memory mapping works for CPU only!
print('Device detected:', device)

print('Streaming model from embedding_tensor.bin...')
model_sparse = SparseTransE(dataset_sparse.n_ent, dataset_sparse.n_rel, emb_dim, storage='embedding_tensor.bin', initialize=False)
print('Initializing and normalizing model...')
# To initialize the model, set initialize=True (default value)
# Or, you can also initialize model from outside source like below:
with torch.no_grad():
  model_sparse.all_emb.copy_(torch.nn.init.xavier_uniform_(torch.randn(dataset_sparse.n_ent + dataset_sparse.n_rel, emb_dim)))
model_sparse.normalize_all_emb()
...
h, t = calculate_hits_at_k_batch(model_sparse, df_test, batch_size=1000, device=device)
